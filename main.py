...
class Ghost:
    ...
    def moveBlinky(self):
        # r,l,u,d
        #blinky will turn whenever colliding with walls, otherwise continue straight
        ...
        if self.direction == 0: # if facing right
            if self.target[0] > self.x_pos and self.turns[0]: # keep going current direction as far as you can to get to target
                ...
                if self.target[1] > self.y_pos and self.turns[3]:#target below, and can turn down
                    ...
                    self.direction = 1
                    self.x_pos -= self.speed
            elif self.turns[0]: # if able to go right, keep going right
                self.x_pos += self.speed
            ...
            if self.target[0] < self.x_pos and self.turns[1]:
                ...
                if self.target[1] > self.y_pos and self.turns[3]:
                    ...
                    self.direction = 0
                    self.x_pos += self.speed
            elif self.turns[1]: # keep going if can
                self.x_pos -= self.speed

        elif self.direction == 2: # if facing up
            if self.target[1] < self.y_pos and self.turns[2]: #keep going up
                self.direction = 2
                self.y_pos -= self.speed
            elif not self.turns[2]:
                if self.target[0] > self.x_pos and self.turns[0]:
...
def checkCollision(score, powerup, power_count, eaten_ghost):
    num1 = (HEIGHT - 50) // 32
    num2 = WIDTH//30
    if 0 < player_x < 870:
...
def getTargets(blink_x, blink_y, blink_dead, ink_x, ink_y, ink_dead, pink_x, pink_y, pink_dead, clyd_x, clyd_y, clyd_dead):
    ...
    if powerup:
        if not blink_dead and not eaten_ghost[0]:
            blink_target = (runaway_x, runaway_y) # run away if powerup active
        elif not blink_dead and eaten_ghost[0]:
        ...
        if not ink_dead and not eaten_ghost[1]:
            ink_target = (runaway_x, player_y) # run away if powerup active
        elif not ink_dead and eaten_ghost[1]:
        ...
        elif not pink_dead and eaten_ghost[2]:
        ...
        if not clyd_dead and not eaten_ghost[3]:
            clyd_target = (450, 450)
        elif not clyd_dead and eaten_ghost[3]:
...
while run:
    ...
    blinky = Ghost(blinky_x, blinky_y, targets[0], ghost_speeds[0], blinky_img, blinky_direction, blinky_dead, blinky_box, 0) # init blinky
    inky = Ghost(inky_x, inky_y, targets[1], ghost_speeds[1], inky_img, inky_direction, inky_dead, inky_box, 1) # init inky
    pinky = Ghost(pinky_x, pinky_y, targets[2], ghost_speeds[2], pinky_img, pinky_direction, pinky_dead, pinky_box, 2) # init pinky
    ...
    if moving: # only allow the player to move after the startup counter
        ...
        if not pinky_dead and not pinky.in_box:
            pinky_x, pinky_y, pinky_direction = pinky.movePinky()
        else:
            pinky_x, pinky_y, pinky_direction = pinky.moveClyde()
        if not inky_dead and not inky.in_box:
            inky_x, inky_y, inky_direction = inky.moveInky()
        else:
            inky_x, inky_y, inky_direction = inky.moveClyde()
        clyde_x, clyde_y, clyde_direction = clyde.moveClyde()

    score, powerup, power_count, eaten_ghost = checkCollision(score, powerup, power_count, eaten_ghost)


    if not powerup: # if collided with a ghost and powerup not active
        if (player_circle.colliderect(blinky.rect) and not blinky.dead) or \
                (player_circle.colliderect(inky.rect) and not inky.dead) or \
                (player_circle.colliderect(pinky.rect) and not pinky.dead) or \
                (player_circle.colliderect(clyde.rect) and not clyde.dead):
            if lives > 0: # if player dies, reset all and start again on new life
                lives -= 1
                startup_counter = 0
                powerup = False
                power_counter = 0
    ...
    if powerup and player_circle.colliderect(clyde.rect) and eaten_ghost[3] and not clyde.dead:
        if lives > 0: # if player dies, reset all and start again on new life
            ...
            moving = False
            startup_counter = 0
    if powerup and player_circle.colliderect(blinky.rect) and not blinky.dead and not eaten_ghost[0]: # if collided with powerup, return ghost to start box
        blinky_dead = True
        eaten_ghost[0] = True
        score += (2 ** eaten_ghost.count(True)) * 100 # increase score based on how many ghosts have been eaten
    if powerup and player_circle.colliderect(inky.rect) and not inky.dead and not eaten_ghost[1]: # if collided with powerup, return ghost to start box
        inky_dead = True
        eaten_ghost[1] = True
        score += (2 ** eaten_ghost.count(True)) * 100 # increase score based on how many ghosts have been eaten
    if powerup and player_circle.colliderect(pinky.rect) and not pinky.dead and not eaten_ghost[2]: # if collided with powerup, return ghost to start box
        pinky_dead = True
        eaten_ghost[2] = True
        score += (2 ** eaten_ghost.count(True)) * 100 # increase score based on how many ghosts have been eaten
    if powerup and player_circle.colliderect(clyde.rect) and not clyde.dead and not eaten_ghost[3]: # if collided with powerup, return ghost to start box
        clyde_dead = True
        eaten_ghost[3] = True
        score += (2 ** eaten_ghost.count(True)) * 100 # increase score based on how many ghosts have been eaten
